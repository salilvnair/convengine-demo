...
drop index if EXISTS ux_ce_config_type_key;
...
CREATE TABLE ce_container_config (
                                     ...
                                     CONSTRAINT ce_container_config_pkey PRIMARY KEY (id)
);
CREATE INDEX idx_ce_container_config_lookup ON public.ce_container_config USING btree (intent_code, state_code, enabled, priority);
...
drop index if EXISTS ix_ce_intent_enabled_priority;

CREATE UNIQUE index if NOT EXISTS ix_ce_intent_enabled_priority ON ce_intent USING btree (enabled, priority, intent_code);
...
drop index if EXISTS idx_ce_intent_classifier_intent_state;

CREATE UNIQUE index if NOT EXISTS idx_ce_intent_classifier_intent_state ON ce_intent_classifier USING btree (intent_code, state_code);
...
drop index if EXISTS idx_ce_llm_log_conversation;
drop index if EXISTS idx_ce_llm_log_intent_state;
...
drop index if EXISTS idx_ce_mcp_tool_enabled;

CREATE UNIQUE index if NOT EXISTS idx_ce_mcp_tool_enabled ON ce_mcp_tool USING btree (enabled, intent_code, state_code, tool_group, tool_code);
...
CREATE UNIQUE index if NOT EXISTS idx_ce_output_schema_lookup ON ce_output_schema USING btree (intent_code, state_code, enabled, priority);
...
drop index if EXISTS idx_ce_policy_priority;
...
drop index if EXISTS idx_ce_prompt_template_lookup;

CREATE UNIQUE index if NOT EXISTS idx_ce_prompt_template_lookup ON ce_prompt_template USING btree (response_type, intent_code, state_code, enabled);
...
drop index if EXISTS idx_ce_response_intent_state;

CREATE UNIQUE index if NOT EXISTS idx_ce_response_intent_state ON ce_response USING btree (intent_code, state_code, enabled, priority);
...
drop index if EXISTS idx_ce_rule_priority;

CREATE index if NOT EXISTS idx_ce_rule_priority ON ce_rule USING btree (enabled, phase, intent_code, state_code, priority);
...
drop index if EXISTS idx_ce_pending_action_lookup;
...
drop index if EXISTS idx_ce_audit_conversation;
...
drop index if EXISTS idx_ce_conversation_history_conv;
...
drop index if EXISTS idx_ce_mcp_db_tool_dialect;
...
drop index if EXISTS idx_ce_mcp_planner_scope;

CREATE UNIQUE index if NOT EXISTS idx_ce_mcp_planner_scope ON ce_mcp_planner USING btree (enabled, intent_code, state_code);
