-- ConvEngine 2.0.7+ canonical MCP seed pack (SQLite)
...
VALUES
('ORDER_DIAGNOSTICS', 'Diagnose submitted order and missing async callback using MCP tools', 40, 1, 'Order Diagnostics',
 'Use MCP HTTP tools to inspect order status and async callback trace, then summarize from MCP final answer.'),
('LOAN_APPLICATION', 'Loan application eligibility and submission workflow', 30, 1, 'Loan Application',
...
VALUES
('ORDER_DIAGNOSTICS', 'UNKNOWN', 'REGEX', '(?i)\b(order|submitted|async|callback|trace|status|diagnostics?)\b', 35, 1,
 'Order diagnostics classifier'),
('LOAN_APPLICATION', 'UNKNOWN', 'REGEX', '(?i)\b(loan|apply loan|personal loan|home loan|eligibility|credit score|loan application)\b', 30, 1,
...
('ORDER_DIAGNOSTICS', 'ANALYZE', 'TEXT', 'DERIVED',
 ...
 NULL, 20, 1, 'Order diagnostics derived response in ANALYZE state'),
('ORDER_DIAGNOSTICS', 'COMPLETED', 'TEXT', 'DERIVED',
 ...
 NULL, 30, 1, 'Order diagnostics completed response derived from MCP final answer'),
('LOAN_APPLICATION', 'IDLE', 'TEXT', 'DERIVED',
 ...
 NULL, 10, 1, 'Loan intake derived prompt with explicit missing required fields'),
('LOAN_APPLICATION', 'ELIGIBILITY_GATE', 'TEXT', 'DERIVED',
 ...
 NULL, 20, 1, 'Loan decision derived from MCP chain'),
('LOAN_APPLICATION', 'COMPLETED', 'TEXT', 'DERIVED',
 ...
 NULL, 30, 1, 'Loan completed response derived from MCP final answer');
...
VALUES
('POST_AGENT_INTENT', 'ORDER_DIAGNOSTICS', 'UNKNOWN', 'REGEX', '.*', 'SET_STATE', 'ANALYZE', 40, 1,
 'Bootstrap ORDER_DIAGNOSTICS into ANALYZE when classifier sets UNKNOWN'),
('POST_AGENT_INTENT', 'ORDER_DIAGNOSTICS', 'IDLE', 'REGEX', '.*', 'SET_STATE', 'ANALYZE', 41, 1,
...
('POST_AGENT_MCP', 'ORDER_DIAGNOSTICS', 'ANALYZE', 'JSON_PATH',
 ...
 'SET_STATE', 'COMPLETED', 42, 1,
 'Move ORDER_DIAGNOSTICS to COMPLETED when context.mcp.finalAnswer exists'),
('POST_AGENT_INTENT', 'LOAN_APPLICATION', 'UNKNOWN', 'REGEX', '.*', 'SET_STATE', 'ELIGIBILITY_GATE', 60, 1,
 'Move LOAN_APPLICATION into ELIGIBILITY_GATE when classifier state is UNKNOWN'),
('POST_AGENT_INTENT', 'LOAN_APPLICATION', 'IDLE', 'REGEX', '.*', 'SET_STATE', 'ELIGIBILITY_GATE', 61, 1,
...
('POST_AGENT_MCP', 'LOAN_APPLICATION', 'ELIGIBILITY_GATE', 'JSON_PATH',
 ...
 'SET_STATE', 'COMPLETED', 62, 1,
 'Move LOAN_APPLICATION to COMPLETED when context.mcp.finalAnswer exists'),
('PRE_RESPONSE_RESOLUTION', 'ORDER_DIAGNOSTICS', 'ANY', 'REGEX', '(?i)\b(reset|restart|start over)\b', 'SET_STATE', 'IDLE', 80, 1,
 'Allow reset for order diagnostics flow'),
('PRE_RESPONSE_RESOLUTION', 'LOAN_APPLICATION', 'ANY', 'REGEX', '(?i)\b(reset|restart|start over)\b', 'SET_STATE', 'IDLE', 81, 1,
...
VALUES
(9101, 'mock.order.status', 'HTTP_API', 'ORDER_DIAGNOSTICS', 'ANALYZE', 1,
 'Fetch order status for diagnostics'),
(9102, 'mock.order.async.trace', 'HTTP_API', 'ORDER_DIAGNOSTICS', 'ANALYZE', 1,
 'Fetch async callback trace for diagnostics'),
(9201, 'loan.credit.rating.check', 'HTTP_API', 'LOAN_APPLICATION', 'ELIGIBILITY_GATE', 1,
 'Step 1: check customer credit rating from credit union API'),
(9202, 'loan.credit.fraud.check', 'HTTP_API', 'LOAN_APPLICATION', 'ELIGIBILITY_GATE', 1,
 'Step 2: if rating > 750, verify customer against fraud API'),
(9203, 'loan.debt.credit.summary', 'HTTP_API', 'LOAN_APPLICATION', 'ELIGIBILITY_GATE', 1,
 'Step 3: if fraud clear, fetch debt/credit summary'),
(9204, 'loan.application.submit', 'HTTP_API', 'LOAN_APPLICATION', 'ELIGIBILITY_GATE', 1,
